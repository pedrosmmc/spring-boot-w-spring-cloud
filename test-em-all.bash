...
# INFO: The waitForService function sends HTTP requests to the supplied URL using curl. Requests are sent repeatedly until curl responds that it got a successful response back from the request. The function waits 3 seconds between each attempt and gives up after 100 attempts, stopping the script with a failure.
...
# INFO: Before the test script runs the test suite, it will check for the presence of a start argument in the invocation of the test script. If found, it will restart the containers.
if [[ $@ == *"start"* ]]
then
...
# INFO: The test script will wait for the composite service to respond with OK.
...
# INFO: All the tests are executed like they were previously. Afterward, the script will tear down the landscape if it finds the stop argument in the invocation parameters.
if [[ $@ == *"stop"* ]]
then
